...
describe('justifyText function', () => {
  it('doit retourner une string', () => {
    const input: string = 'Lorem ipsum dolor sit amet.';
    const output: string = justifyText(input);
    expect(typeof output).toBe('string');
  });

  it('doit simplement retourner la ligne sans la modifier si elle est plus petite que 80 caractères', () => {
    const input: string = 'Lorem ipsum dolor sit amet.';
    const output: string = justifyText(input);
    expect(output).toBe(input);
  });

  it('ne doit pas couper des mots', () => {
    const input: string = 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed in purus sed ligula semper luctus.';
    const output: string = justifyText(input);
    const lines: string[] = output.split('\n');
    const lastLine: string = lines[lines.length - 1];
    const lastWord: string = lastLine.split(' ')[lastLine.split(' ').length - 1];
    expect(lastWord.length).toBeGreaterThan(0);
  })

  it('chaque ligne doivent faire une longueure de 80 caractères sauf la dernière', () => {
    const text = 'This is a long paragraph that needs to be justified. It should be split into multiple lines. The last line should not be justified.';
  ...
  it('test avec le input et output envoyés par Tictactrip', () => {
