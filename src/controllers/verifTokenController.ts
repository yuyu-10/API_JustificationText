...
const checkLimit = async (res: Response, userToken: IUser, text: String) => {
    try {
        ...
        const userHistory = await History.findOne({ id_user: id_user, date: getDate()});
        ...
        const newUserHistory = await History.findOne({ id_user: id_user, date: getDate()});
        const canUpdateWords = await checkWords(text, newUserHistory.words);
        if (canUpdateWords) {
            const newWords = newUserHistory.words + text.split(' ').length;
            const updateWords = await History.findOneAndUpdate({ id_user: id_user, date: getDate() }, { words: newWords});
        } else {
